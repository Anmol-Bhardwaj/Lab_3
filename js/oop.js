...
function makingHamBrgr(bun, cheese, sauce, meatPatty, pattyLayers, garnish, exToppings) {
    let hamburger = {
        bun: bun,
        cheese: cheese,
        sauce: sauce,
        pattyLayers: pattyLayers,
        garnish: garnish,
        exToppings: exToppings,

        analyze: function() {
            let prompt = `Hello Customer, Your order has been placed. Your hamburger contains: Bun : ${this.bun}, Cheese : ${this.cheese}, Sauce : ${this.sauce}, Number of Patty Layers : ${this.pattyLayers}, Garnish : ${this.garnish}`;
            if (this.exToppings.length > 0) {
                prompt += `, Extra Toppings: ${this.exToppings.join(', ')}`;
            }
            return prompt;
        }
    };
